...
fn test_cursored_investments_empty_investor_returns_generation_zero() {
    ...
    let investor = Address::generate(&env);
    let page = client.get_investor_investments_cursor(&investor, &None, &0u32, &10u32, &None);
...
fn test_cursored_investments_boundary_offset_at_and_past_total_count() {
    ...
    let at_boundary =
        client.get_investor_investments_cursor(&investor, &None, &3u32, &10u32, &None);
    ...
    let past_boundary =
        client.get_investor_investments_cursor(&investor, &None, &u32::MAX, &10u32, &None);
...
fn test_cursored_investments_concurrent_insert_is_detected_as_unstable() {
    ...
    let page1 = client.get_investor_investments_cursor(&investor, &None, &0u32, &2u32, &None);
...
fn test_cursored_investments_retry_with_fresh_generation_succeeds() {
    ...
    let restarted = client.get_investor_investments_cursor(&investor, &None, &0u32, &2u32, &None);
...
fn test_cursored_investments_invalid_cursor_generation_is_rejected() {
    ...
    let result =
        client.try_get_investor_investments_cursor(&investor, &None, &0u32, &10u32, &Some(999u64));
    let err = result.err().expect("expected UnstableCursor error");
    let contract_error = err.expect("expected contract error");
    assert_eq!(contract_error, QuickLendXError::UnstableCursor);
}
...
fn test_cursored_investments_large_result_capped_to_max_query_limit() {
    ...
    let page =
        client.get_investor_investments_cursor(&investor, &None, &0u32, &(count * 10), &None);
...
fn test_cursored_investments_repeated_identical_calls_are_idempotent() {
    ...
    let first = client.get_investor_investments_cursor(&investor, &None, &1u32, &2u32, &None);
